...
DOPlotlySingleStationUI <- function(id){
  ...
  tagList(
    wellPanel(
      ...
      br(),hr(),br(),
      fluidRow(
        column(6, h5('All DO records that fall below the criteria for the ',span(strong('selected site')),' are highlighted below.'),
               div(style = 'height:150px;overflow-y: scroll', tableOutput(ns('DOrangeTableSingleSite')))),
        column(6, h5('Station Exceedance Rate:'),
               uiOutput(ns('stationDOExceedanceRateSelect_UI')),
