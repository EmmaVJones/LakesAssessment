...
stationTable <- read_csv('processedStationData/draft2020data/RegionalResultsLake_BRRO.csv') %>%
  as_tibble() %>%
  filter(!is.na(OBJECTID))


lakeStations <- read_csv('processedStationData/draft2020data/lakeStations2020draft.csv') %>%
  # until all station data filled in filter out stuff that will bomb out app
  filter(FDT_STA_ID != '4ADAN187.94')
  ...
  for(k in 1:length(unique(conventionals_Lake$ID305B))) {
    ...
    for(j in 1:length(unique(stationDataDailySample$FDT_STA_ID))){
      ...
      StationTableResults <- cbind(StationTableStartingData(stationData), tempExceedances(stationData),
                                   ...
                                   TP_Exceedances(stationData), chlA_Exceedances(stationData),
...
rm(thermo);rm(stationData);rm(StationTableResults);rm(stationDataDailySample);rm(lake_filter);rm(conventionals_Lake);rm(AU)
```
...
stationTableResultsWithComments <- left_join(stationTableResults, 
                                             dplyr::select(stationTable, STATION_ID, COMMENTS), by = 'STATION_ID') %>%
